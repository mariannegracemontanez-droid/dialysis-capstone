...
/// The one place the Admin panel reports the outcome of an action.
///
...
/// inside an Admin modal was landing underneath that modal, unread.
///
...
/// touched.
///
///   Admin page -> Admin modal route -> notice barrier -> notice card
///
/// ## The four types
///
...
enum AdminNoticeType { success, error, info, warning }
...
class AdminNotice {
  ...
  static Future<T?> _present<T>({
    ...
    if (overlay == null) {
      assert(() {
        debugPrint('AdminNotice: no overlay available for "${spec.message}"');
        return true;
      }());
    ...
    entry = OverlayEntry(
      builder: (_) => _NoticeLayer(spec: spec, onClosed: close),
    );
...
class _NoticeSpec {
  ...
  bool get isConfirmation => type == AdminNoticeType.warning;
  ...
  Color get accent {
    switch (type) {
      ...
      case AdminNoticeType.info:
        return AppTheme.blue1;
      case AdminNoticeType.warning:
  ...
  Color get accentSoft {
    switch (type) {
      case AdminNoticeType.success:
        return AppTheme.accentGreenSoft;
      case AdminNoticeType.error:
        return AppTheme.dangerSoft;
      case AdminNoticeType.info:
        return AppTheme.accentBlueSoft;
      case AdminNoticeType.warning:
        return AppTheme.accentOrangeSoft;
    }
  }
...
class _NoticeLayerState extends State<_NoticeLayer>
  ...
  final FocusScopeNode _focusScope = FocusScopeNode(debugLabel: 'AdminNotice');
              ...
              Positioned.fill(
                child: GestureDetector(
                  ...
                  child: FadeTransition(
                    opacity: _curve,
                    child: const ColoredBox(color: Color(0x4D1F2D3D)),
                  ),
                ),
              ),
        ...
        child: Material(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                if (spec.autoDismissAfter != null) _countdownBar(spec),
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                        ...
                        child: Icon(spec.icon, color: spec.accent, size: 21),
                      ),
                ...
                if (spec.confirmLabel != null) _actions(spec),
              ],
            ),
          ),
        ),
  ...
  Widget _countdownBar(_NoticeSpec spec) {
    final reduceMotion =
        MediaQuery.maybeOf(context)?.disableAnimations ?? false;

    if (reduceMotion) {
      return Container(height: 3, color: spec.accentSoft);
    }
      ...
      builder: (context, value, _) {
        return SizedBox(
          ...
          child: LinearProgressIndicator(
            value: value,
            backgroundColor: spec.accentSoft,
  ...
  Widget _actions(_NoticeSpec spec) {
    final confirmStyle = spec.destructive || spec.type == AdminNoticeType.error
