...
class DashboardService {
  ...
  final totalDonations = (donations as List<dynamic>)
      .map((item) => double.tryParse((item as Map<String, dynamic>)['amount']?.toString() ?? '0') ?? 0)
  ...
  Future<List<CenterModel>> fetchCenters() async {
    ...
    return list
        .map((postgres) => CenterModel.fromJson(postgres as Map<String, dynamic>))
        .toList();
  }
 ...
 Future<void> createCenter({
  ...
  await _supabase.from('clinics').insert({
    ...
    'address': address,
    'city': city,
    'requirements': [requirements], // ✅ ARRAY FIX
    'latitude': latitude,
    'longitude': longitude,
 ...
 Future<void> updateCenter({
  ...
  final status = computeStatus(slotAvailable);
  await _supabase.from('clinics').update({
    'name': name,
    'address': address,
    'city': city,
